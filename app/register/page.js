...
export default function RegisterPage() {
  ...
  const C = {
    ...
    btnSecondary: { background: 'none', border: 'none', color: '#7a82a0', fontSize: '13px', cursor: 'pointer', padding: '8px 0', textDecoration: 'underline' },
  ...
  if (submitted) return (
    <div style={C.page}>
      <nav style={C.nav}><div style={C.navInner}><a href="/" style={C.logo}><span style={{ color: '#4a9eff' }}>Assam</span> Innovates</a></div></nav>
      <div style={{ ...C.card, textAlign: 'center', paddingTop: '60px' }}>
        <div style={{ fontSize: '56px', marginBottom: '20px' }}>🎉</div>
        <div style={{ fontSize: 'clamp(20px,5vw,28px)', fontWeight: 800, color: '#2dd4a0', marginBottom: '10px' }}>Welcome to the Tinkerspace!</div>
        <p style={{ fontSize: '15px', color: '#7a82a0', lineHeight: 1.7, marginBottom: '28px' }}>
          You are now on the Innovation Map at <strong style={{ color: '#dde1f0' }}>{form.town}</strong>. Other makers in your area can find and connect with you.
        </p>
    ...
    <div style={C.page}>
      ...
      <nav style={C.nav}>
        <div style={C.navInner}>
          ...
          <div className="desk-nav" style={{ display: 'flex', gap: '20px', fontSize: '13px', color: '#7a82a0' }}>
      ...
      <div style={C.card}>
        ...
        <p style={C.sub}>Join 240+ makers across Assam. Your profile will appear on the Innovation Map and connect you with local tinkers.</p>
        ...
        {step === 1 && (
          <div>
            ...
            <div style={C.field}>
              ...
              <textarea style={C.textarea} placeholder="Tell the community a bit about yourself and what you're working on..." value={form.bio} onChange={e => update('bio', e.target.value)} />
            </div>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ textAlign: 'center', marginTop: '10px' }}>
              <button style={C.btnSecondary} onClick={() => setStep(1)}>← Back</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,220px),1fr))', gap: '10px', marginBottom: '20px' }}>
              {skillLevels.map(s => (
                <div key={s.id} onClick={() => update('skillLevel', s.id)}
                  style={{
            ...
            <button style={C.btnPrimary} onClick={nextStep}>Continue →</button>
            <div style={{ textAlign: 'center', marginTop: '10px' }}>
              <button style={C.btnSecondary} onClick={() => setStep(2)}>← Back</button>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '24px' }}>
              {interestOptions.map(interest => (
                <div key={interest} onClick={() => toggleInterest(interest)}
                  style={{
                    ...
                    color: form.interests.includes(interest) ? '#2dd4a0' : '#7a82a0',
                    cursor: 'pointer',
                  }}>
                  {interest}
                </div>
            ...
            <div style={{ backgroundColor: '#1a1d2e', border: '1px solid #2a2f4a', borderRadius: '8px', padding: '14px 16px', marginBottom: '20px' }}>
              <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', color: '#7a82a0', marginBottom: '10px' }}>Your profile summary</div>
              {[['Name', form.name], ['Town', form.town], ['Level', form.skillLevel]].map(([k, v]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #2a2f4a', fontSize: '13px' }}>
                  <span style={{ color: '#7a82a0' }}>{k}</span>
                  <span style={{ color: '#dde1f0' }}>{v}</span>
                </div>
              ))}
            </div>
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Saving your profile...' : 'Join the Tinkerspace →'}
            </button>
            <div style={{ textAlign: 'center', marginTop: '10px' }}>
              <button style={C.btnSecondary} onClick={() => setStep(3)}>← Back</button>
            </div>
            <p style={{ textAlign: 'center', fontSize: '11px', color: '#4a5070', marginTop: '14px' }}>
              Your data is stored securely. We never share your email address.
            </p>
          </div>
        )}
      </div>
